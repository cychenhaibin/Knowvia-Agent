...
func TestStoreContract(t *testing.T) {
	...
	backends := []struct {
		name  string
		build func(*testing.T) contractStore
	}{
		{
			name: "memory",
			build: func(t *testing.T) contractStore {
				t.Helper()
		...
		{
			name:  "postgres",
			build: newContractPostgresStore,
		},
	}
	...
	for _, backend := range backends {
		...
		t.Run(backend.name, func(t *testing.T) {
			...
			s := backend.build(t)
			testAuthContract(t, s)
			testChatContract(t, s)
			testKnowledgeContract(t, s)
			testChatModelContract(t, s)
			testRunContract(t, s)
			testSkillContract(t, s)
			testMirrorTaskContract(t, s)
		})
	}
}
